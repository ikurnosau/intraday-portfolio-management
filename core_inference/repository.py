...
class Repository:
    ...
    def add_bar(self, data: dict[str: Any]):
        symbol = data["symbol"]
        with self._lock:
            self.bar_received_at[symbol] = time.time()
            self.bars_and_quotes[symbol].loc[len(self.bars_and_quotes[symbol])] = {
                "open": data["open"],
                "high": data["high"],
                "low": data["low"],
                "close": data["close"],
                "volume": data["volume"],
                "date": data["date"],
                "bid_price": data.get("bid_price", self.bid_price[symbol]),
                "bid_size": data.get("bid_size", self.bid_size[symbol]),
                "ask_price": data.get("ask_price", self.ask_price[symbol]),
                "ask_size": data.get("ask_size", self.ask_size[symbol]),
            }
    ...
    def get_asset_dfs(self) -> dict[str: pd.DataFrame]:
        with self._lock:
            return {
                symbol: self.bars_and_quotes[symbol]
                .tail(self.required_history_depth)
                .copy()
